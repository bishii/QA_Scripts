...
#matrix pins 1-8
IO.setup(37,IO.OUT)  #initialize GPIO12 as an output.
IO.setup(35,IO.OUT)  #initialize GPIO22 as an output.
IO.setup(33,IO.OUT)
IO.setup(31,IO.OUT)
IO.setup(29,IO.OUT)
IO.setup(23,IO.OUT)
...
def PORT(pin):  #assigning GPIO state by taking 'pin' value
	if(pin&0x01 == 0x01):
		IO.output(26,0)   #if bit0 of 8bit 'pin' is true pull PIN21 low
	else:
		IO.output(26,1)   #if bit0 of 8bit 'pin' is false pull PIN21 high
	if(pin&0x02 == 0x02):
		IO.output(22,0)   #if bit1 of 8bit 'pin' is true pull PIN20 low
	else:
		IO.output(22,1)   #if bit1 of 8bit 'pin' is false pull PIN20 high
	if(pin&0x04 == 0x04):
		IO.output(19,0)   #if bit2 of 8bit 'pin' is true pull PIN26 low
	else:
		IO.output(19,1)   #if bit2 of 8bit 'pin' is false pull PIN26 high
	if(pin&0x08 == 0x08):
		IO.output(16,0)
	else:
		IO.output(16,1)
...
def PORTP(pinp):    #assigning GPIO logic for positive terminals by taking 'pinp' value
	if(pinp&0x01 == 0x01): 
		IO.output(18,1)     #if bit0 of 8bit 'pinp' is true pull PIN12 high
	else:
		IO.output(18,0)     #if bit0 of 8bit 'pinp' is false pull PIN12 low
	if(pinp&0x02 == 0x02):
		IO.output(33,1)     #if bit1 of 8bit 'pinp' is true pull PIN22 high
	else:
		IO.output(33,0)     #if bit1 of 8bit 'pinp' is false pull PIN22 low
	if(pinp&0x04 == 0x04):
		IO.output(31,1)     #if bit2 of 8bit 'pinp' is true pull PIN27 high
	else:
		IO.output(31,0)     #if bit2 of 8bit 'pinp' is false pull PIN27 low
	if(pinp&0x08 == 0x08):
		IO.output(24,1)
	else:
		IO.output(24,0)
	...
	if(pinp&0x40 == 0x40):
		...
		IO.output(24,0)
	if(pinp&0x80 == 0x80):
		IO.output(26,1) #if bit7 of 8bit 'pinp' is true pull PIN18 high
	else:
		IO.output(26,0) #if bit7 of 8bit 'pinp' is false pull PIN18 low
