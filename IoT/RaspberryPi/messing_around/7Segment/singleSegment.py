...
while True:
	...
	displayed = []
	for a in givenAnswer:
			
		GPIO.output(theDigits[dec],GPIO.LOW)
		
		print("working on: %s"  % a)

		theSet = num[str(a)]
		for segnum in range(0,7):
			ans = digits[segnum]	
			if theSet[segnum] == 1:
				GPIO.output(ans,GPIO.HIGH)
			else:
				GPIO.output(ans,GPIO.LOW)
		
		GPIO.output(theDigits[dec],GPIO.HIGH)
		dec += 1
